...
//View Report(Opens PDF in Browser)
router.get("/view-report", authenticate, requireTeacher, async (req, res) => { // Added requireTeacher
    try {
        const report = await OCresult.findOne({ teacherId: req.userId }).sort({ createdAt: -1 }); // Corrected: Use req.userId
